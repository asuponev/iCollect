    ...
    >
      <CardActionArea onClick={() => navigate(`/collections/${collectionId}/items/${_id}`)}>
        <CardContent sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          ...
          <Typography fontSize={16} color="#142339" fontWeight={700} gutterBottom>{title}</Typography>
          <ItemTags tags={tags} compact={true} />
          <Grid container wrap="nowrap" mt={1} sx={{ color: "#797E85" }}>
            <Typography variant="overline" noWrap>
              {collection.title}
            </Typography>
            <Typography variant="overline" mx={1}>—</Typography>
            <Typography variant="overline" noWrap>
              by {collection.authorId.firstName} {collection.authorId.lastName}
            </Typography>
          </Grid>
        </CardContent>
      </CardActionArea>
